...
async def generate_personalized_exam(
    ...
    print(f"🔍 [DEBUG] request.question_count = {request.question_count}")
    print(f"🔍 [DEBUG] 推荐分布 = {analysis['recommended_distribution']}")
    print(f"🔍 [DEBUG] 初始分布 = {distribution}")

    # 确保题目总数正确
    if request.question_count:
        total = sum(distribution.values())
        print(f"🔍 [DEBUG] 需要调整: total={total}, target={request.question_count}")
        if total != request.question_count:
            ...
            print(f"🔍 [DEBUG] 调整后分布 = {distribution}")
