...
async def test_provider_by_id(
    ...
    """用数据库中存储的真实密钥测试已有的AI提供商"""
    result = await db.execute(select(AIProvider).where(AIProvider.id == provider_id))
    provider = result.scalar_one_or_none()
    if not provider:
        raise HTTPException(404, "配置不存在")

    real_key = decrypt_api_key(provider.api_key)
    if not real_key or real_key == "***":
        return AIProviderTestResponse(success=False, message="API密钥解密失败，请重新编辑保存密钥")
    ...
    try:
        ...
        client = OpenAI(
            api_key=real_key,
            base_url=provider.base_url or "https://dashscope.aliyuncs.com/compatible-mode/v1"
        )
        response = client.chat.completions.create(
        ...
        return AIProviderTestResponse(
            ...
            response_time=round(response_time, 2), test_output=test_output
        )
    except Exception as e:
        return AIProviderTestResponse(success=False, message=f"连接失败: {str(e)}")
