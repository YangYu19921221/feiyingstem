...
2. 如果有多个发音,返回最常用的英式发音(RP/Received Pronunciation)
3. 确保使用标准的IPA符号
4. 不要包含任何解释或额外文字
...
要求:
1. 音标: 使用标准IPA格式的英式发音(RP),用斜杠包围,如 /ˈæpl/
2. 释义:
...
"""
    ...
    async def generate_speech(self, text: str) -> bytes:
        """
        生成语音 - 优先使用讯飞TTS，降级到阿里云CosyVoice
        ...
        # 优先使用讯飞TTS
        try:
            from app.services import iflytek_tts_service
            iflytek_config = await iflytek_tts_service.get_config()
            if iflytek_config:
                return await iflytek_tts_service.generate_speech(text)
        except Exception as e:
            print(f"讯飞TTS失败，尝试降级到阿里云: {e}")
