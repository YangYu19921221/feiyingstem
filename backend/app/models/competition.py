...
class LeaderboardSnapshot(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    season_id = Column(Integer, ForeignKey("competition_seasons.id", ondelete="CASCADE"), nullable=False)
    org_id = Column(Integer, nullable=False, default=1, server_default="1")  # 榜单快照按机构分(多租户);索引由init_db迁移建
...
class CompetitionQuestionSet(Base):
    ...
    org_id = Column(Integer, nullable=True)  # 多租户: NULL=平台共享题库,非NULL=机构自建;索引由init_db迁移建
