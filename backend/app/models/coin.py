...
规则(2026-08-08 重定义,金额以 coin_service 常量为准):
- 完成当日老师布置的全部作业 → +1 币(当天无作业则不发;老师取消/关闭的任务不进分母);
- 当日班级词量榜第一(单词王)→ +1 币,与任务币**叠加**(当王又做完任务 = 2 枚)。
一天封顶 2 枚(DAILY_CAP)。两者均每日结算、幂等(靠 CoinTransaction.dedup_key
唯一约束防重复发放)。只发「当天」的:今天补做昨天的任务不给币(迟做要补走教师端
手动加币,有 PIN 有流水有操作人)。
...
class CoinTransaction(Base):
    ...
    source:
      - task      系统:完成当日全部作业 +1
      - word_king 系统:当日单词王 +1,与任务币**叠加**(2026-08-08 起从 +2 且互斥
                  改成 +1 且叠加;当王又做完任务=2)。一天封顶 2 枚
      - manual    老师手动增减(可正可负)
      - redeem    兑换消耗(负数)
    金额以 coin_service 的 TASK_REWARD / WORD_KING_REWARD / DAILY_CAP 为准,
    别在这里写死数字 —— 这段注释就曾因规则重定义而过时。
