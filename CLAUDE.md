...
- ✅ 音标视频配套课件(2026-09-09): 音标视频此前**只能传视频本身**,老师讲课那份 PPT
  ...
  **`<video>` 必须在所有布局下留在树里同一位置只换 className** —— 大小两块都 absolute
  ...
  ④**小窗大小可鼠标拖拉(2026-09-09,用户两次提)**: 内侧竖边只改宽(ew-resize)、
  内侧斜角横竖都拖(nesw/nwse-resize),双击任一还原,尺寸存 localStorage。
  16:9 锁死只算宽度,高度自己跟着走;**斜角取「主导方向」**(位移大的那个轴)而不是
  两轴相加,否则斜着拖会冲过头。把手长在**朝舞台里侧**那两边(朝外没地方拖),
  方向随吸附角翻转(贴右边时左拖才是变大)。三个坑:
  (a)**偏好值与显示值必须分两个 state**: 只用一个 state 时舞台一变小就把它改写了 ——
  实测拖到 634 存住,刷新后讲义还没展开(舞台只有视频那么高)夹取成 200 并写回,
  等讲义展开也回不去了。偏好值只有用户拖动才改,夹取只在渲染时算
  (b)拖宽走**内联 style**,那就**不能再挂 min-w/max-w 类** —— 它们是独立属性,
  会把内联宽度夹回 240~480
  (c)上限要同时受舞台**宽和高**约束: 只卡宽度的话,矮而宽的窗口上拖到七成宽,
  16:9 的高度早超出舞台了。指针捕获(setPointerCapture)必须加,否则拖过视频区域
  时事件被 <video> 控件吃掉;把手 onPointerDown 要 stopPropagation,不然和 framer 的
  拖动抢同一次按下。**底部两角吸附位改 bottom-14**: 小窗一拖大就压住翻页条,
  而它默认就落在右下角(不是用户拖歪的)。顶部两角仍会盖住讲义顶栏,那是用户
  自己拖上去的,不额外处理
