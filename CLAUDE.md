...
- ✅ 音标视频配套课件(2026-09-09): 音标视频此前**只能传视频本身**,老师讲课那份 PPT
  ...
  **PPT 转换依赖服务器装 LibreOffice**(services/office_convert.py 调 soffice,
  ...
  ③**讲义与视频必须同屏,不是二选一**(第一版做成全屏弹层盖住视频+暂停,用户当场指出
  是错的): 老师讲到哪页学生手边翻到哪页。横屏/桌面用 `landscape:flex-row` 左视频 58%
  右讲义,可「放大讲义」切 36/64;竖屏手机 `portrait:` 视频钉顶 aspect-video、讲义占下方。
  **`<video>` 必须在两种布局下留在树里同一位置,只换 className** —— 写成两个分支各放
  一个 <video>,展开讲义那一下就重挂、进度归零。弹层 z 要 [70]:学生端底部导航 z-60、
  宠物浮标 z-50 都会压住竖屏时的翻页条。键盘箭头翻页要判焦点在 <video> 上时不接管
  (那时箭头是快退/快进);Esc 先收讲义再关播放器
