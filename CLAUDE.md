...
- ✅ 传说宝可梦 + 20 个新家族(2026-08-11): 种族 40→60。12 个普通家族专挑
  ...
  8 只传说分两档,门槛是**累计去重学词数**: 准传说(急冻鸟/闪电鸟/火焰鸟/水君)2500,
  顶级传说(梦幻/超梦/烈空坐/阿尔宙斯)5000。真源 core/pet_species(SEMI_LEGEND_WORDS/
  LEGEND_WORDS/TIER_*),前端 config/petSpecies 必须同名同值。
  **传说走独立队伍格**(MAX_LEGEND_SLOTS=2,2500 开第1格、5000 开第2格),不占普通 5 格——
  ...
  **`collection.pets` 是混装的(普通+传说),画格子禁止按下标直取**: 传说排在领养顺序
  ...
  另两处「够门槛 ≠ 领得到」的坑: ①学 2500 词只开 1 格,第二只准传说够词但没格子,
  图鉴卡别写「可以收服」(要按 canAdoptLegend 分开写)②传说进度条/档位名要以
  **下一档阈值**为基准算(拿 learned_words 直除阈值,刚满 2500 时第 2 格会显示已走一半;
  用已开格数判断档位名会把「还差 2500 词开启顶级传说」写成「准传说」)
