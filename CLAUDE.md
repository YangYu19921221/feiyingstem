...
- ✅ 音标视频按讲师分类(2026-09-17): 一个机构几位老师各讲一套,学生进去看到的是
  ...
  **讲师是自由文本不是 users 外键**(2026-09-17 用户明确要求「上传的时候选老师分类,
  ...
  ①**「留空」在这里是有效操作** = 取消归属改回「全校通用」,所以 VideoUpdate.lecturer
  不能写 min_length=1,判空必须走**显式分支**(`if "lecturer" in data` 而不是
  `and data["lecturer"]`)—— 与 CLAUDE.md 那条 AI 密钥被清空的坑同源,只是方向相反:
  那次错在"空串被当成要改",这次会错在"空串被当成不改",两种都要走显式分支
  ...
  **存量零影响**:迁移刻意**不按 created_by 回填**(那是"谁上传的"不是"谁讲的",
  ...
  (上传前填,整批同一个值)+ 列表上方讲师 chip。测试 tests/test_phonetic_lecturer.py(37 例,
