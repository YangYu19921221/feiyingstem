...
- ✅ 音标视频按讲师分类(2026-09-17): 一个机构几位老师各讲一套,学生进去看到的是
  ...
  **讲师是自由文本不是 users 外键**(2026-09-17 用户明确要求「上传的时候选老师分类,
  ...
  十个坑(除①②外都是两轮评审里实测复现出来的 —— 拿自由文本当分组维度,「看不见的字符」和「改了却没生效」是主要风险面):
  ...
  **单条编辑也走同一套**(传 changing_ids=[自己]): 该讲师只有这一个视频时必须改得动,
  ...
  按 rows 的 org 取并集。⚠️ 加类似的"看已有值再决定写什么"的逻辑时都要想这一层 ——
  admin 的无过滤上下文会让"已有值"变成全平台的
  ...
  ⑥**归一键必须做 NFKC + 删零宽字符**,只做 casefold + 去空白不够: 中文输入法全角状态
  敲出的是「Ｍiss Ｌucy」(U+FF2D),从 Word/PPT 复制的名字常夹带 U+200B/FEFF ——
  两者在屏幕上**完全看不出差别**,却会让一个人在学生端变成两个 chip。
  ⚠️ 这类不可见字符在源码里**一律写 `\uXXXX` 转义,禁止写字面量**: 字面量在编辑器里
  看不见、review 看不出,连拿文本匹配去改都会失败(实测:自己写的那版自己改不动)。
  ...
  **存量零影响**:迁移刻意**不按 created_by 回填**(那是"谁上传的"不是"谁讲的",
  ...
  (上传前填,整批同一个值)+ 列表上方讲师 chip。测试 tests/test_phonetic_lecturer.py(30 例,
