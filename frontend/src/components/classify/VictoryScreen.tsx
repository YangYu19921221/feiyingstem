 ...
 *
 * 三档：
 * - perfect (100): 持剑英雄站崖顶 + 金色日出
 * - great (80-99): 笑容英雄 + 黄昏花海
 * - retry (<80): 蓄力英雄 + 晨雾山峦
 *
 * 与旧版差异：
 * - 主视觉是 gpt-image-2 生成的角色立绘背景，不再是 emoji + 渐变
 * - 取消 SunRays / LightningRing / 漫天彩带等 AI slop
 * - 文字层用实色 + 描边，不用 gradient-clip text
 * - 入场动效：英雄从下浮入 + 标题切片淡入，一次性，不持续转
 */
...
interface Theme {
  ...
  subtitle: string;
  image: string;
  // 半透明渐变蒙层，让顶部标题区可读、底部按钮区干净
  topVeil: string;
  bottomVeil: string;
  // 文字与按钮主色（OKLCH 暖色，从图片色温里走出来）
  titleColor: string;
  titleStroke: string;
  accent: string;     // 主按钮 / 数字色
  accentHover: string;
  chipBg: string;     // 数据 chip 背景
  chipText: string;
}
...
// 标题切片：每字 60ms 错峰浮入
function SlicedTitle({ text, color, stroke }: { text: string; color: string; stroke: string }) {
  return (
    <h1
      className="font-display text-3xl md:text-5xl font-black tracking-[0.18em] flex justify-center flex-wrap"
      style={{
        color,
...
export default function VictoryScreen({
  ...
  // 分数从 0 滚到 score（1.2s），cubic ease-out
  useEffect(() => {
    const duration = 1200;
  ...
  return (
    <div className="fixed inset-0 z-40 overflow-y-auto bg-black">
      {/* 背景立绘：cover 全屏，Ken-Burns 轻微缓慢推近（10s 一次性） */}
      <motion.div
        key={theme.key}
      ...
      <div className="relative z-10 min-h-full flex flex-col px-5 py-8 md:py-12 max-w-2xl mx-auto w-full">
        ...
        <div className="text-center mt-2 mb-auto">
          ...
          <SlicedTitle text={theme.title} color={theme.titleColor} stroke={theme.titleStroke} />
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
