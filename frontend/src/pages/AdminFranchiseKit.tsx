 ...
 *     此前 09-11 曾由「¥12,000 = 原价 ¥20,000 打 6 折」改为「¥24,000 无折扣」,
 *     本次是在 ¥24,000 这个标准价上重新给六折 —— 注意基数是 24,000 不是 20,000),
 *     含 100 张学生学习卡(每张半年 / 180 天),超出按 ¥130/张;
 *     飞鹰英语专属内容 +¥16,000(09-11 由 ¥40,000 下调);
 *     ⚠️ **六折只打在基础合作费上**,超额卡/续卡/专属内容/教材/带教培训全按标准价,
 *     协议里必须写明这句,否则日后必生争议;且写成「限期优惠 + 不构成续签价格承诺」,
 *     避免被解读成长期标准价(第三条第 1 款留了签约日期空栏现填)
 *     ⚠️ 方案里有几个数字是**从基础合作费算出来的**,改价必须连带重算(本次已改 4 处):
 *     数字牌(¥14,400 / 每张卡 14400/100=¥144 / 每天 144/180≈¥0.8)+ 费用表那一行
 *     + 投入产出那段(1.44 万;并保留标准价口径的对照,否则读者无法核对折扣);
 *     ⚠️ 投入产出里那句**比较性结论会随价格翻转**:折后 1.44 万 < 一本书流水 2–3 万,
 *     原文「大致相当」已改成「低于」;续卡单张 ¥130 相对 ¥144 只低约一成
 *     (相对标准价 ¥240 才是「降约一半」),改价时这两句必须一起重读
 ...
 *     第三条第 3 款 + 第九条第 2 款必须成对出现 —— 只写费用条会与「一年一签」自相矛盾)
 *     协议与销售方案两份文档口径必须一致,改一处必须改另一处
 *     ⚠️ **第五条「年度续卡量与正常经营」是 2026-09-18 新插入的**,后面 7 条编号
 *     全部顺移(原五→六 … 原十一→十二)。它是**双向条件**:乙方每年续满 50 张
 *     (首年已含 100 张视为达标)→ 甲方不得取消授权资格 / 不得动区域保护 /
 *     不得拒绝同等条件续签。加这条时踩到的三件事:
 *     ①**插条款会牵动交叉引用**,改完必须逐个核 —— 第七条(原六)引「第三条第 N 款」、
 *       第九条(原八)的续签款、本文件头都带条款号;当时顺手修了一处**既有错位**:
 *       带教培训在第三条是第 6 款,原文却引「第 5 款」(那是配套教材)
 *     ②**「不得取消资格」不能写成无条件的** —— 必须留违约出口(第五条第 4 款列了
 *       转授权/破解导出/逾期付款/重大事故/停业九十日五种),否则乙方倒卖账号甲方也动不了他
 *     ③**未达标的后果要与「终止服务」分开**: 未续满只降级为非独家或到期不续签,
 *       **不单方掐掉协议期内的服务**,已售出的学习卡在有效期内继续可用 ——
 *       学生卡还没到期就停服会把乙方的学员一起坑掉,那是真实纠纷来源
    ...
    <article className="fk-serif text-[14px] leading-7 text-slate-900">
      ...
      <Clause no="五" title="年度续卡量与正常经营">
        ...
        <p>
          ...
          <strong>正常经营</strong>。乙方正常经营期间,甲方
          <strong>不得单方取消或收回乙方的授权资格</strong>、不得取消或缩小第四条约定的
          区域保护范围、不得在该区域内发展其他合作点,亦
          <strong>不得拒绝乙方在同等条件下续签本协议</strong>(与第九条第 2 款一并适用)。
        </p>
        <p>
          3. 乙方某一协议年度未达到第 1 款续卡量的,甲方应书面(含微信、邮件等可留痕方式)
          通知乙方,乙方可在收到通知后<strong>三十日内补足差额</strong>,补足的视为达标。
          逾期未补足的,甲方有权将第四条的区域保护调整为<strong>非独家</strong>
          (即甲方可在该区域内发展其他合作点),或于协议到期时不予续签;
          <strong>但甲方不因此单方终止协议期内的系统服务</strong>,
          乙方已购买的学习卡在协议期内按各自有效期继续可用
          (学习卡有效期跨出协议期的部分,按第九条第 3 款处理)。
        </p>
        <p>
          4. 乙方有下列情形之一的,不适用本条第 2 款对甲方的限制,甲方有权按第十条追究
          违约责任,并可书面通知终止本协议:
        </p>
        <p className="pl-4">
          (1) 未经甲方书面同意,转授权、分销、转让本协议项下权利,或以甲方、
          「飞鹰英语」名义对外签约;
        </p>
        <p className="pl-4">
          (2) 复制、破解、导出本系统或教学内容并向第三方提供(含出借管理账号、
          批量导出题库与教材内容);
        </p>
        <p className="pl-4">(3) 逾期支付应付费用超过三十日;</p>
        <p className="pl-4">
          (4) 因乙方原因发生重大教学安全事故、虚假宣传或违法经营,损害甲方品牌声誉;
        </p>
        <p className="pl-4">(5) 乙方连续停止实际经营超过九十日。</p>
      </Clause>

      <Clause no="六" title="乙方义务与经营规范">
        <p>
    ...
    <article className="text-[13.5px] leading-6 text-slate-800">
      ...
      <PitchSection no="九" title="为什么可以放心合作">
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>
            <strong>授权资格有保障,不是我们说了算</strong>:第二年起每年续满 50 张卡即视为正常经营,
            期间<strong>我们不得取消你的授权资格、不得动区域独家、不得拒绝同等条件续签</strong>——
            这是写进协议第五条的对甲方的约束,不是口头承诺。
          </li>
          <li>
            <strong>一年一签</strong>:先跑通再续签,机构风险可控;正常履约享同等条件优先续签权。
            某年没续满也<strong>不会中途停你的服务</strong>,已买的学习卡在有效期内照用,
            最多是区域改为非独家或到期不续。
          </li>
